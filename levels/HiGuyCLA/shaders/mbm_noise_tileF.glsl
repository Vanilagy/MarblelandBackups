...
void main() {
	...
	// Additionally, the model position is added so there aren't obvious repeats in
	// tile shading when an interior is copied.
	float shade = noise(floor(scaled_uv) + floor(model_position.xy));
	...
	float cosNtheta = clamp(dot(-normal_color, light_normal), 0.5, 1.0) + 0.3;
